...
class SoakAnalysisTests(unittest.TestCase):
    def test_flat_series_passes(self) -> None:
        ...
        jittered = [
            ...
            if sample.monotonic >= WARMUP_SEC["steady"]
            else sample
            for idx, sample in enumerate(samples)
        ]
        verdict = analyse(jittered, "steady")
    ...
    def test_linear_leak_2mb_per_hour_fails_m1(self) -> None:
        verdict = analyse(_series(slope_mb_per_hour=2.0), "steady")
    ...
    def test_step_growth_30mb_fails_m2_not_m1(self) -> None:
        ...
        verdict = analyse(stepped, "steady")
        self.assertFalse(verdict.passed)
        self.assertIn("M2", verdict.reason)
        self.assertNotIn("M1", verdict.reason)

    def test_slow_leak_0_9mb_per_hour_fails_m2(self) -> None:
        verdict = analyse(_series(hours_after_warmup=28.0, slope_mb_per_hour=0.9), "steady")
        self.assertFalse(verdict.passed)
        self.assertNotIn("M1", verdict.reason)
        self.assertIn("M2", verdict.reason)

    def test_handle_leak_one_per_reconnect_fails_m3(self) -> None:
        samples = _series(hours_after_warmup=2.0, profile="churn")
        leaked = [
            _sample(sample.monotonic, 100.0, handles=20 + idx)
            if sample.monotonic >= WARMUP_SEC["churn"]
            else sample
            for idx, sample in enumerate(samples)
        ]
        verdict = analyse(leaked, "churn")
        self.assertFalse(verdict.passed)
    ...
    def test_warmup_hour_excluded_from_baseline(self) -> None:
        ...
        verdict = analyse(samples, "steady")
        self.assertTrue(verdict.passed, verdict.reason)

    def test_warmup_differs_per_profile(self) -> None:
        samples = [_sample(float(idx * SAMPLE_INTERVAL_SEC), 100.0) for idx in range(241)]

        churn_verdict = analyse(samples, "churn")
        self.assertTrue(churn_verdict.passed, churn_verdict.reason)

        steady_verdict = analyse(samples, "steady")
        self.assertFalse(steady_verdict.passed)
        self.assertIn("M5", steady_verdict.reason)

    def test_churn_60min_run_is_analysable(self) -> None:
        interval = 3572.0 / 119.0
        samples = [_sample(idx * interval, 100.0) for idx in range(120)]

        verdict = analyse(samples, "churn")
        self.assertFalse(verdict.passed)
        self.assertIn("M5", verdict.reason)
        self.assertGreaterEqual(verdict.samples_used, 2)
    ...
    def test_sleep_gap_20pct_fails_m5(self) -> None:
        ...
        gapped = [
            sample
            for idx, sample in enumerate(samples)
            if sample.monotonic < WARMUP_SEC["churn"] or idx % 5 != 0
        ]
        verdict = analyse(gapped, "churn")
        self.assertFalse(verdict.passed)
        self.assertIn("M5", verdict.reason)
    ...
    def test_load_samples_reads_jsonl(self) -> None:
        sample = _sample(WARMUP_SEC["steady"], 100.0)
