...
void test_recovery_from_invalid_changed_offset_reports_change()
{
   CBrokerTime clock;
   CFakeClockSource src;
   const datetime utc = MakeTime(2026, 7, 27, 9, 15, 2);
   src.Set(utc + 7200, utc);
   AssertTrue(clock.Init(GetPointer(src)), "test_recovery_from_invalid_changed_offset_reports_change init");

   src.Set(utc + 6900, utc);
   AssertTrue(!clock.Refresh(), "test_recovery_from_invalid_changed_offset_reports_change first_bad");
   src.AdvanceSeconds(91);
   AssertTrue(!clock.Refresh(), "test_recovery_from_invalid_changed_offset_reports_change invalidated");
   AssertTrue(!clock.IsValid(), "test_recovery_from_invalid_changed_offset_reports_change is_invalid");

   src.Set(utc + 10800 + 120, utc + 120);
   AssertTrue(clock.Refresh(), "test_recovery_from_invalid_changed_offset_reports_change changed");
   AssertTrue(clock.IsValid(), "test_recovery_from_invalid_changed_offset_reports_change valid");
   AssertEqualInt(clock.OffsetSeconds(), 10800, "test_recovery_from_invalid_changed_offset_reports_change offset");
   AssertEqualInt(clock.PreviousOffsetSeconds(), 7200, "test_recovery_from_invalid_changed_offset_reports_change previous");
   AssertTrue(clock.LastChangeUtc() > 0, "test_recovery_from_invalid_changed_offset_reports_change last_change");
}
...
void test_refresh_throttles_samples_20s()
{
   CBrokerTime clock;
   CFakeClockSource src;
   const datetime utc = MakeTime(2026, 7, 27, 9, 15, 2);
   AssertTrue(InitClock(clock, src, 7200), "test_refresh_throttles_samples_20s init");
   src.Set(utc + 3600 + 1, utc + 1);
   clock.Refresh();
   src.Set(utc + 3600 + 2, utc + 2);
   clock.Refresh();
   src.Set(utc + 3600 + 3, utc + 3);
   AssertTrue(!clock.Refresh(), "test_refresh_throttles_samples_20s no_early_change");
   AssertEqualInt(clock.OffsetSeconds(), 7200, "test_refresh_throttles_samples_20s unchanged");
}
...
void test_broker_day_start_across_dst_23h_and_25h()
{
   ...
   clock.Refresh();
   src.Set(MakeTime(2026, 3, 29, 10, 0, 21) + 10800, MakeTime(2026, 3, 29, 10, 0, 21));
   clock.Refresh();
   src.Set(MakeTime(2026, 3, 29, 10, 0, 41) + 10800, MakeTime(2026, 3, 29, 10, 0, 41));
   ...
   autumn_src.Set(MakeTime(2026, 10, 25, 10, 0, 21) + 7200, MakeTime(2026, 10, 25, 10, 0, 21));
   autumn_clock.Refresh();
   autumn_src.Set(MakeTime(2026, 10, 25, 10, 0, 41) + 7200, MakeTime(2026, 10, 25, 10, 0, 41));
...
void RunAllTests()
{
   ...
   RecordRan("test_recovery_from_invalid_changed_offset_reports_change");
   ...
   RecordRan("test_refresh_throttles_samples_20s");
