...
| **rev.2** | **2026-07-27** | **เปลี่ยนชุด symbol เป็น 8 คู่ · ตัด BTCUSD ออก · เพิ่ม FX majors + XAGUSD** |
...
| D4 | Symbol | **8 คู่** ดูตาราง §1.2 — 🟠 **`USDCNY.iux` ไม่มีในโบรกเกอร์** |
...
| 8 | `XAGUSD.iux` | ✅ | ✅ | contract size ต้องยืนยัน — ดู §3 |
...
| ข | **ตัดทิ้ง** เหลือ 7 คู่ | ทางที่ง่ายที่สุด |
...
| R6/R7 HWM ต้องเดินต่อเสาร์-อาทิตย์ | ✅ **ยกเลิก** — ทั้ง 7 คู่ปิดสุดสัปดาห์ |
...
### 2.2 ★ ทั้ง 7 คู่มี USD อยู่ข้างหนึ่งทุกตัว

```
EURUSD  USDJPY  GBPUSD  AUDUSD  USDCAD  XAUUSD  XAGUSD
...
### 2.3 ★ 7 คู่ แต่เดิมพันอิสระจริงประมาณ 4 ก้อน
...
| โลหะ | `XAUUSD` `XAGUSD` | ทอง-เงิน correlation สูง |
...
|-------|--------|-----------|
| quote = USD → ใช้ตรงได้ | `EURUSD` `GBPUSD` `AUDUSD` `XAUUSD` `XAGUSD` | ตรงไปตรงมา |
...
→ **เปลี่ยนเป็น: `USDJPY` · `USDCAD` · `XAGUSD`** (สองตัวแรกคือเคสแปลงสกุล ตัวหลังคือเคส contract size ผิดปกติ)
...
**7 symbol × 1 TF (M1) × ~10 ปี** — ไม่ใช่ 7 × 7
...
ตัด BTC ออกแล้วก็ไม่ช่วย — และ **`XAGUSD` อาจแพงกว่า BTC ที่ตัดออกไปด้วยซ้ำ**
...
| `XAGUSD.iux` | 5,000 oz² | 0.001 | $5.00² | 0.7 pt = **$0.0007** |

¹ ขึ้นกับ conversion rate ขณะนั้น ตัวเลขนี้ประกอบการอธิบายเท่านั้น
² **ต้องยืนยัน** — โบรกเกอร์ใช้ 5,000 oz หรือ 1,000 oz ต่อ lot ต่างกัน **5 เท่า** ในทุนที่ต้องใช้

**ทุกค่าแคบกว่า spread ของตัวเอง → `raw_lot` ปัดลงเป็น 0 → reject ทุก intent ทั้ง 7 คู่ ตลอดกาล**
...
| `XAGUSD.iux` | $0.30 | **~$4,286** (ถ้า 5,000 oz) · ~$857 (ถ้า 1,000 oz) |

**ตัวที่บังคับทุนทั้งชุด = `XAGUSD`** → ~$4,300/บัญชี (หรือ ~$1,430 ถ้า silver 1,000 oz ทำให้ทองกลับมาเป็นตัวคุม)
...
|---|----------|--------------|------|
| **A** | **บัญชี Cent** (ถ้า IUX มี) — 1 lot = 1/100 standard | $10 = 1,000 หน่วยบัญชี → **FX majors 5 คู่ผ่านหมด** · ทองต้อง ~$15 · เงินต้อง ~$43 | ✅ **แนะนำ** ถ้าจะลงเงินจริงตอนนี้ · ครบ 7 คู่ควรมี **$50–100** จริง |
| **B** | เพิ่มทุน standard account | ต้อง ~$600 (FX) / ~$1,430 (+ทอง) / ~$4,300 (+เงิน) ต่อบัญชี × ≥2 บัญชี | ✅ ตรงไปตรงมา |
| **C** | คงทุน $10 = **demo เท่านั้น** | พิสูจน์ท่อ Phase 1 ได้ครบ ไม่ต้องแก้อะไร | ✅ ยอมรับได้ ถ้ายังไม่รีบขึ้น live |
| **D** | ตัด `XAGUSD` ออกเพื่อลดทุนที่ต้องใช้ | เพดานลงมาที่ ~$1,430 (ทอง) | 🟡 เป็นการตัดสินเชิงกลยุทธ์ ไม่ใช่เชิงเทคนิค — เจ้าของตัดสิน |
...
| `XAGUSD` SL $0.30 | $15.00 = **150% ของบัญชี** | **429 เท่า** — margin call ไม้เดียว |
...
ต้องได้ `reject` **ห้ามได้ `volume_min`** · ต้องครอบคลุม `XAGUSD` ด้วย (เคสรุนแรงที่สุด)
...
- [ ] **ยืนยัน contract size ของ `XAGUSD.iux`** — 5,000 หรือ 1,000 oz (สวิงทุนที่ต้องใช้ 5 เท่า)
- [ ] **ตัดสิน D5** — A / B / C / D
...
- [ ] `SPEC-020` — สูตร R1 ที่แก้แล้ว + test conversion 3 เคส (`USDJPY` `USDCAD` `XAGUSD`)
