...
> ### ⚠️ แก้ 2026-08-01 — **การวินิจฉัยด้านล่างนี้ผิด** · ดู [`021`](../handoff/to-codex/021-แก้คำสั่ง-S1-S2-เพิ่ม-S4-S5.md) + [audit](reviews/SPEC-001-timing-audit.md)
>
> `/1000` **ไม่ใช่ต้นเหตุ** — `floor(e/1000) >= K ⟺ e >= K*1000` ตรงเป๊ะ ไม่มีอคติ
> ต้นเหตุจริงคือ **ฐานรีเซ็ตที่ `:612` + `EventSetTimer(1)`** → **(b) คือตัวแก้ ไม่ใช่ (a)**
> · **S2 ยกเลิกของเดิม** (ไม่มีอคติ −999 ms ให้ไล่) · **เพิ่ม S4 · S5**
...
| # | ทำ |
|---|-----|
| ~~**S2**~~ | ~~ไล่ทุกจุดที่เรียก `ElapsedSec()` — มีอคติ −0 ถึง −999 ms เสมอ~~ · 🔴 **ยกเลิก 2026-08-01 — อคตินั้นไม่มีอยู่จริง** → เปลี่ยนเป็น: ยืนยันหรือโต้แย้ง [audit §1](reviews/SPEC-001-timing-audit.md) ทั้ง **11 จุด** โต้แย้งต้องมาพร้อมเลขคณิต |
| **S4** 🔴 | **ใหม่** — `FarmExecutor.mq5:109` ใช้ `>` กับค่า `floor()` → brain timeout ตั้ง 10s **ยิงจริง 11.000s** · เมื่อ SPEC-023 ขึ้นจะเป็น R16 → **ตกเกณฑ์ Phase 1 exit "SafeMode ≤ 10s"** |
| **S5** 🟠 | **ใหม่** — `Wire.mqh:786` `NowTick() < m_next_connect_tick` เทียบ unsigned ดิบ ซึ่ง **S1(d) ห้ามไว้เอง** · ไม่ค้างถาวร แต่ backoff ถูกข้าม 1 ครั้งทุก ~49.7 วัน |
