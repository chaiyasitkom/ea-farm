...
## 8. ★ กฎการส่งค่าเมื่อ "ไม่รู้" — ใช้กับทุก message ทุก field

> **เพิ่ม 2026-07-30** หลัง [review SPEC-063](reviews/SPEC-063.md) พบ bug 3 ตัว (B1 · B2 · B3)
> ที่มีต้นตอเดียวกัน: **ไม่เคยเขียนไว้ว่า "ไม่มีค่า" ต้องส่งอะไร** ผู้เขียนโค้ดจึงเดา
> และเดาไปทาง *"ใส่อะไรก็ได้ที่ผ่าน schema"* ซึ่งเป็นทางที่แย่ที่สุด

### 8.1 ต้นไม้ตัดสิน — มีแค่ 3 ทางเท่านั้น

```
field นี้ไม่มีค่าจะส่ง
   │
   ├─ อยู่ใน "required" ของ schema?
   │     └─ ✅ ใช่ → ★ ไม่ส่ง message นี้เลย
   │              (ไม่ใช่ส่งด้วยค่ามั่ว — ดู §8.3)
   │
   ├─ optional และ schema มี "null" ใน type?
   │     └─ ✅ ใช่ → ส่ง null
   │
   └─ optional และ schema ไม่มี "null"?
         └─ ★ ละ key ทิ้งไปเลย — ห้ามส่ง null
```

| ทาง | เมื่อไร | ตัวอย่างจริง |
|-----|---------|-------------|
| **ละ key** | optional · `type` ไม่มี `"null"` | `HELLO.broker_time` — `$ref` ไป `broker_time_info` ที่เป็น `"type":"object"` → `null` = **validation FAIL → gateway ปฏิเสธ HELLO ทั้งใบ** อาการจะออกมาเป็น *"EA ต่อไม่ติดเลย"* ซึ่งไล่หาสาเหตุยากกว่าที่ควรมาก |
| **ส่ง `null`** | optional · `type` มี `"null"` | `HEARTBEAT.wire.broker_utc_offset_sec` — `["integer","null"]` และเขียนไว้ชัดว่า `null` แปลว่า EA ไม่รู้เวลาตัวเอง |
| **ไม่ส่ง message** | field อยู่ใน `required` | `ts_server` / `ts_sent` ตอน `CBrokerTime.IsValid()==false` |

### 8.2 ★★ ห้ามใช้ค่า sentinel ที่ทับกับค่าจริงได้

**นี่คือกฎที่ละเมิดบ่อยที่สุดและเจ็บที่สุด** — ห้ามใช้ `0` · `-1` · `""` · `[]` ·
timestamp คงที่ · หรือค่า "กลางๆ ที่ดูปลอดภัย" แทนคำว่า "ไม่รู้"

| ค่าที่เคยมีคนใส่ | ทำไมผิด |
|-----------------|---------|
| `broker_utc_offset_sec: 0` | `0` เป็น offset ที่ **ถูกต้องได้จริง** (โบรกเกอร์ที่ตั้งเวลาเป็น UTC) → brain แยกไม่ออกระหว่าง *"broker อยู่ที่ UTC"* กับ *"EA ไม่รู้เวลาตัวเอง"* |
| `ts_sent: 1700000000` (epoch คงที่) | **ผ่าน pattern `ts_utc` ทุกประการ** → gateway รับเข้าไปโดยไม่มีอะไรฟ้อง · ULID ที่ฝัง timestamp นี้จะเรียงมาก่อนทุก session ที่ถูกต้อง → dedupe/ordering พัง · latency ติดลบหลายปี |
| `correlation: 0` เมื่อข้อมูลไม่พอ | `0` = "ไม่ correlate" = **ปลอดภัยที่สุด** ในสายตากฎ P4 → ข้อมูลหายกลายเป็นไฟเขียว ([G1](06-gap-audit.md) · [SPEC-061](specs/SPEC-061-stale-data-guard.md)) |

**หลักการเดียวที่ใช้กับทุกเคส:**

> **"ไม่รู้" ต้องเห็นชัดว่าไม่รู้ · ห้ามแปลงเป็นค่าที่หน้าตาปกติ**
> ค่าที่ดูเหมือนถูกแต่ผิด อันตรายกว่าค่าที่เห็นชัดว่าพัง — เพราะไม่มีใครไปตรวจ

### 8.3 ทำไม "ไม่ส่ง message" ถึงเป็นคำตอบที่ถูก ไม่ใช่การยอมแพ้

EA ที่ส่งอะไรไม่ได้เพราะขาดข้อมูลบังคับ **คืออาการที่ถูกต้อง**:
gateway จะเห็น heartbeat หายแล้วตัด session (P11) → brain รู้ทันทีว่าบัญชีนี้ใช้ไม่ได้
ซึ่งดีกว่าปล่อยข้อมูลที่ดูดีแต่ผิดเข้า audit trail ที่ต้องใช้สืบสวนตอนเงินหาย

**แต่ต้องเห็นได้** — เงียบหายไปเฉยๆ ไม่พอ ทุกจุดที่ตัดสินใจ "ไม่ส่ง" ต้องมี:

1. **counter** ที่นับจำนวนครั้ง (เช่น `messages_dropped_no_time`)
2. **log WARN ครั้งแรกของช่วง** — ไม่ใช่ทุกครั้ง (ไม่งั้น log ท่วมตอนพัง)
3. รายงาน counter ขึ้นสายทันทีที่ส่งได้อีกครั้ง

### 8.4 ฝั่งรับ — ห้ามเติมค่าให้เอง

ผู้รับ (gateway · brain · EA) ที่เจอ key หายหรือ `null` **ห้ามใส่ default แล้วเดินต่อ**
ถ้า field นั้นจำเป็นต่อการตัดสินใจ → **ปฏิเสธ message นั้นและ log** ให้เห็นว่าปฏิเสธเพราะอะไร
· default ที่ฝั่งรับเติมเองคือ bug class เดียวกับ §8.2 แค่ย้ายที่เกิดเหตุ
