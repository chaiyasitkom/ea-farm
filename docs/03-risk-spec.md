...
ถ้าตีความว่า **notional** กฎนี้ block ทุกอย่างที่ทุนระดับไหนก็ตาม:

| farm equity | P3 = 2% | EURUSD 0.01 lot (notional ~$1,080) | ผล |
|-------------|---------|-----------------------------------|-----|
| $60 (2 บัญชี × $30) | $1.20 | เกิน **900 เท่า** | reject ทุกไม้ |
| $6,000 | $120 | เกิน **9 เท่า** | reject ทุกไม้ |
| $100,000 | $2,000 | เพดาน = 0.02 lot | เล็กจนไร้ประโยชน์ |

**→ ตีความแบบ notional ใช้ไม่ได้** ไม่ใช่แค่ที่ทุนน้อย

**ตีความที่สอดคล้องกับตัวเลข 2.0%:** exposure วัดเป็น **risk-normalized** —
คือ "ถ้า SL ของทุกไม้ที่มีสกุลนั้นเป็นขาโดนชนหมด จะเสียกี่ % ของ farm equity"

ยืนยันด้วยตัวอย่างในเอกสารเอง: *"5 บัญชี แต่ละบัญชี long EURUSD 0.4% = รวม 2.0% ต่อ EUR"*
— `0.4%` จะบวกกันได้เป็น `2.0%` ก็ต่อเมื่อทั้งคู่เป็นหน่วยเดียวกันคือ **% ของ equity**
ซึ่งเข้ากับ risk-normalized ไม่ใช่ notional

| | P3 ที่ 2% ภายใต้ risk-normalized |
|---|---|
| 1 ไม้เต็มขนาดตาม R1 | 0.35% ของบัญชี |
