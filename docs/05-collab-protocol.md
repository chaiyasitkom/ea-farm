...
| **`handoff/to-codex/**`** | **Claude เขียน** | Codex **อ่านอย่างเดียว ห้ามแก้** |
| **`handoff/to-claude/**`** | **Codex เขียน** | Claude **อ่านอย่างเดียว ห้ามแก้** |
| `handoff/README.md` · `handoff/archive/**` | Claude (README) · ย้ายเข้า archive ทำได้ทั้งคู่ | — |

### 📮 Handoff Bus — ช่องทางคุยกันโดยไม่ต้องผ่านเจ้าของ (เพิ่ม 2026-07-31)

**กฎเต็มอยู่ที่ [`handoff/README.md`](../handoff/README.md)** — สรุปสามข้อที่ต้องจำ:

1. **กล่องละคนเขียน** — ไม่มีวัน conflict · ตอบ = สร้างไฟล์ใหม่ในกล่องตัวเอง พร้อม `replies_to:`
2. **เริ่มทุก session ด้วย `git pull` แล้วดูกล่องตัวเอง** — ว่าง = ไม่มีอะไรค้าง ไปดู work-order ต่อ
3. **`blocking: true` มาก่อนทุกอย่าง** — ห้ามเริ่มงานอื่นจนกว่าจะตอบ

> **ทำไมถึงต้องมี:** ที่ผ่านมาทุกข้อความวิ่งผ่านเจ้าของด้วยการ copy-paste
> → คำถามหาย · ต้องอธิบายบริบทซ้ำ · และมีอย่างน้อยหนึ่งครั้งที่การตกหล่นของบริบท
> ทำให้ไล่บั๊กผิดทางไปหลายชั่วโมง

⚠️ **บัสไม่ใช่ spec** — ถ้าคุยกันแล้วได้ข้อสรุปที่เปลี่ยนพฤติกรรมของระบบ
**ต้องไปแก้ `docs/specs/` ด้วย** แล้วอ้างถึงในข้อความตอบ
