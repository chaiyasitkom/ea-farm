...
| **D4** | ✅ **ปิดแล้ว rev.4 — 6 คู่ × 7 timeframe** (`XAGUSD` ตัด rev.3 · `USDCNY` ตัด rev.4) · ปลดล็อก SPEC-007 (ต้องดาวน์โหลด `USDJPY` ก่อน) · **เพิ่มงาน:** R5/R10/R11 ต้องเป็นตารางต่อ symbol · ทุกคู่มี USD → USD เป็นคอขวดของ P3 · เดิมพันอิสระจริง ~4 ก้อนไม่ใช่ 6 → P4 bind บ่อยกว่า P3 และ P5 = 3 หลวมเกิน |
| **D5** | ✅ **ปิดแล้ว: demo-first** (rev.5) — IUX ไม่มีบัญชี cent → $30 standard เทรดไม่ได้เลย ตรวจครบทุกคันโยกในสูตร R1 แล้วตันหมด · Phase 0–2 ไม่ต้องใช้เงินจริง จึง**ไม่ทำให้ช้าลง** · ทุน live ตัดสินตอนจบ Phase 2 |

### ~~🔴~~ ✅ พบระหว่างคำนวณ D5: P3/P4 ไม่ระบุหน่วยของ exposure → D9 (**ปิดแล้ว 2026-07-30**)

> **ตัดสินแล้ว → [ADR-004](decisions/ADR-004-exposure-unit.md):** exposure = **risk-normalized**
> · เพดาน 2.0% ต่อสกุล · **USD (สกุลบัญชี) 5.0%** · สูตรผูกพันอยู่ใน ADR + [SPEC-024](specs/SPEC-024-currency-exposure.md)
> · ช่องว่างคู่กัน **D10** (hedge ข้ามบัญชี) ปิดด้วย [ADR-005](decisions/ADR-005-cross-account-hedge.md) — กฎใหม่ **P12**

เพดาน P3 เขียนว่า `2.0% ของ farm equity` แต่ currency decomposition ให้ผลเป็น **notional**
สองหน่วยนี้เทียบกันตรงๆ ไม่ได้ · ถ้าตีความแบบ notional กฎจะ **reject ทุกไม้ที่ทุนระดับไหนก็ตาม**
(แม้ farm $100,000 ก็ได้เพดานแค่ 0.02 lot)

นี่เป็นช่องว่างคนละแบบกับ G1–G12: ไม่ใช่ "ลืมทำ" แต่เป็น **กฎที่เขียนไว้แล้วแต่ตีความได้สองทาง
และทางหนึ่งทำให้ระบบหยุดสนิท** — ต้องปิดก่อน SPEC-024 ดู [03-risk-spec §P3](03-risk-spec.md)
